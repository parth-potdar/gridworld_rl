"""Implement SARSA with gridworld"""
import numpy as np
...
-> BUT, this is on-policy, so uses same epsilon-greedy policy to update state and learn Q
"""
num_eps = 5000
learning_rate = 0.1
epsilon = 1.0
...
for i in range(num_eps):
    ...
    while not done:
        ...
        action = epsilon_greedy(current, epsilon, Q_table, env.num_actions)
        ...
        # SARSA: Use current policy to get next action at new state
        next_action = epsilon_greedy(obs, epsilon, Q_table, env.num_actions)

        # apply Q-learning TD update
        Q_table[current[0]][current[1]][action] += learning_rate * (
            reward + discount * Q_table[obs[0]][obs[1]][next_action]
            - Q_table[current[0]][current[1]][action] 
        )
